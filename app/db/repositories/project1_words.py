...
class Project1WordsRepository(BaseRepository):

    async def get_all_dictwords(self, *, word: str) -> List[str]:
        dictword_rows = await queries.p1_get_all_dictwords(self.connection)
    ...
    async def get_dictwords_by_word(self, *, word: str) -> List[WordOutWithIdDate]:
        word_rows = await queries.p1_get_dictwords_by_word(
            self.connection,
            word=word,
        )
        if word_rows:
    ...
    async def get_dictword_by_id(self, *, id: int) -> WordOutWithIdDate:
        word_row = await queries.p1_get_dictword_by_id(
            self.connection,
            id=id,
        )
        if word_row:
    ...
    async def get_1_random_dictword(self) -> WordOutWithIdDate:
        ...
        raise EntityDoesNotExist(
            "Couldn't get a random word".format(id),
        )
    ...
    async def create_new_dictword(
        ...
        *,
        ...
        async with self.connection.transaction():
            word_row = await queries.p1_create_new_dictword(
                self.connection,
                word=w.word,
                type=w.type,
                fullword=w.fullword,
    ...
    async def update_dictword_row_by_id(  # noqa: WPS211
        ...
        *,
        ...
        async with self.connection.transaction():
            word_updated = await queries.p1_update_dictword_row_by_id(
                self.connection,
                id=id,
    ...
    async def delete_dictword(self, *, id: int) -> str:
        r = await queries.p1_delete_dictword_by_id(
            self.connection,
            id=id,
        )
        if r:
